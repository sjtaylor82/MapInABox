...
class MapNavigator(NavMixin, WalkMixin, ToolsMixin, FreeMixin, LookupsMixin, wx.Frame):
    ...
    def _show_poi_in_listbox(self):
        ...
        for poi in self._poi_list:
            ...
            if (self._transit and
                    ...
                    "Control+Enter" not in label):
                label = label + " — Ctrl+Enter for transit info"
            labels.append(label)
    ...
    def _show_favourites(self):
        ...
        if not entries:
            self._status_update("No favourites saved. Press Control+Shift+F to add one.", force=True)
            return
    ...
    def _on_listbox_char(self, event):
        ...
        no_mod = (not event.ShiftDown() and not event.ControlDown()
    ...
    def _on_keyboard(self, event):
        ...
        no_mod = (not event.ControlDown() and not event.AltDown()
    ...
    def _show_transit_drill_dialog(self, child_pois, title, hint, focus_index=0):
        ...
        def _hook(evt):
            ...
            if ctrl and alt and kc in (ord('F'), ord('f')):
    ...
    def _report_mark_to_destination(self, slot):
        ...
        if not mark:
            self._status_update(f"mark {slot} not set. Press Control M then {slot}.", force=True)
            return
    ...
    def on_key(self, event):
        ...
        # handlers from firing on Ctrl+P, Shift+X, etc.
        no_mod = not shift and not control and not alt


        dist, _ = _nearest_city(self._city_lats, self._city_lons, self.lat, self.lon)
        on_land = _IS_LAND(self.lat, self.lon) or dist < 0.3

        # Ctrl+Alt+F — find food along active transit line.
        # Must come BEFORE the Ctrl+F favourites check below, which otherwise
        # matches first because its condition does not exclude alt being held.
        if control and alt and not shift and (key == ord('F') or key == ord('f')):
            self._tool_find_food()
            return

        # Favourites — works in any mode when a coordinate/POI is available.
        if control and not alt and key in (ord('F'), ord('f')):
            if shift:
                self._add_current_favourite()
            else:
        ...
        if getattr(self, '_free_mode', False):
            ...
            if key == wx.WXK_DOWN:
                self._free_step(-1); return
            if control and key == wx.WXK_LEFT:
                self._free_snap_cross(); return
            if control and key == wx.WXK_RIGHT:
                self._free_snap_cross(); return
            if key == wx.WXK_LEFT:
        ...
        if shift and not control and key == wx.WXK_F6:
        ...
        if shift and not control and key == wx.WXK_F10:
            self._game.repeat_target()
            return
        if control and key == wx.WXK_F10:
            if self._session and self._session.active:
                self._session.stop()
                self._session = None
                self._game._timeout_cb = None
        ...
        if self.street_mode and key in (page_up, page_down):
            ...
            elif not control and not shift and not alt:
        ...
        if control and shift and not alt and (key == ord('S') or key == ord('s')):
        ...
        if control and shift and alt and (key == ord('S') or key == ord('s')):
        ...
        if not self.street_mode and not getattr(self, '_walking_mode', False):
            ...
            if shift and not control and (key == ord('T') or key == ord('t')):
            ...
            if control and alt and key == wx.WXK_UP:
                self._jump_nearest_land("north"); return
            if control and alt and key == wx.WXK_DOWN:
                self._jump_nearest_land("south"); return
            if control and alt and key == wx.WXK_LEFT:
                self._jump_nearest_land("west"); return
            if control and alt and key == wx.WXK_RIGHT:
        ...
        if shift and not control and (key == ord('L') or key == ord('l')):
        ...
        if alt and not control and not shift and key in (ord('1'), ord('2'), ord('3')):
            self._report_mark_to_destination(int(chr(key)))
            return
        if control and not shift and not alt and (key == ord('M') or key == ord('m')):
            self._prompt_mark_slot(remove=False)
            return
        if control and shift and not alt and (key == ord('M') or key == ord('m')):
            self._prompt_mark_slot(remove=True)
            return

        if control and not shift and not alt and (key == ord('D') or key == ord('d')):
            self._set_map_destination()
            return

        if control and alt and not shift and (key == ord('D') or key == ord('d')):
            self._prompt_destination_mark_slot()
            return

        if control and alt and not shift:
        ...
        if not self.street_mode:
            if shift and not control and not alt and (key == ord('P') or key == ord('p')):
                self._announce_postcode();  return


        if self.street_mode:
            if control and (key == ord('W') or key == ord('w')):
            ...
            if no_mod and (key == ord('S') or key == ord('s')):
                self._street_search()
                return
            if control and (key == ord('G') or key == ord('g')):
        ...
        if (not self.street_mode and not getattr(self, '_walking_mode', False)
                and map_arrow and control and not shift and not alt):
            direction = {
                wx.WXK_UP: "north",
                wx.WXK_DOWN: "south",
                wx.WXK_LEFT: "west",
